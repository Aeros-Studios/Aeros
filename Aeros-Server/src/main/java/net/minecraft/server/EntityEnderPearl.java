...
public class EntityEnderPearl extends EntityProjectile {
	...
	protected void a(MovingObjectPosition movingobjectposition) {
		...
		if (blockPosition != null) {
			...
			boolean collides = AerosConfig.pearlPassthroughTripwire && block == Blocks.TRIPWIRE
					|| AerosConfig.pearlPassthroughCobweb && block == Blocks.WEB
					|| AerosConfig.pearlPassthroughBed && block == Blocks.BED
					|| AerosConfig.pearlPassthroughFenceGate
					...
					|| AerosConfig.pearlPassthroughSlab && (block == Blocks.STONE_SLAB
		...
		if (!this.world.isClientSide) {
			if (entityliving instanceof EntityPlayer) {
				...
				if (entityplayer.playerConnection.a().isConnected() && entityplayer.world == this.world
					...
					if (AerosConfig.antiEnderPearlGlitch) {
					...
					if (!teleEvent.isCancelled() && !entityplayer.playerConnection.isDisconnected()) {
						...
						Aeros.getInstance().getLagCompensator().registerMovement(player, location); // Nacho
