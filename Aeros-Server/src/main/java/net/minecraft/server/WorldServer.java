...
import dev.aeros.async.entitytracker.MultithreadedEntityTracker;
import dev.aeros.config.AerosConfig;
import dev.aeros.world.WorldTicker;
...
public class WorldServer extends World implements IAsyncTaskHandler {
	...
	public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
		...
		// Aeros - async entity tracking
		if (AerosConfig.disableTracking) {
			this.tracker = new EntityTracker(this);
		} else if (AerosConfig.fullAsyncTracking) {
	...
	public void doTick() {
		...
		if (this.getGameRules().getBoolean("doMobSpawning")
			...
			// Aeros start - disable mob spawning based on tps and server load
			if (!(AerosConfig.limitedMobSpawns
					&& MinecraftServer.getServer().recentTps[0] < AerosConfig.limitedMobSpawnsThreshold) // check
																											// if
																											// tps
																											// is
																											// low
					&& !(AerosConfig.stopMobSpawnsDuringOverload && lastTickOverload)) { // check if server is
																							// overloaded
				// Aeros end
