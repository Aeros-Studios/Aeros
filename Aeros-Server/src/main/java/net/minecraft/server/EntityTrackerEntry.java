...
// Aeros - sendPacket methods have been replaced with the queuePacket method
public class EntityTrackerEntry {
	...
	// Aeros - re add removed method used by Citizens
	public EntityTrackerEntry(Entity entity, int b, int c, boolean flag) {

		// Aeros Start - get the entity's tracker from it's dimension
	...
	public void track(List<EntityHuman> list) {
		...
		if (this.tickCount % this.c == 0 || this.tracker.ai || this.tracker.getDataWatcher().a()) {
			...
			if (this.tracker.vehicle == null) {
				...
				if (this.u) {
					...
					if (d4 > d3 * d3 || d4 > 0.0D && this.tracker.motX == 0.0D && this.tracker.motY == 0.0D
						...
						this.broadcastInternal(
								new PacketPlayOutEntityVelocity(this.tracker.getId(), this.motionX, this.motionY,
										this.motionZ));
					}
				}
				...
				if (flag2) {
					this.broadcastInternal(
							new PacketPlayOutEntity.PacketPlayOutEntityLook(this.tracker.getId(), (byte) i,
	...
	private void b() {
		...
		if (datawatcher.a()) {
			this.broadcastIncludingSelfInternal(
