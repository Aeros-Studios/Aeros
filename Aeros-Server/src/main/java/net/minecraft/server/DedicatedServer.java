...
import dev.aeros.commons.IPUtils;
...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
	...
	public DedicatedServer(joptsimple.OptionSet options, Thread thread1) { // Aeros - backport modern tick loop
		super(options, Proxy.NO_PROXY, DedicatedServer.a, thread1);
		// CraftBukkit end
		if (!AerosConfig.disableInfiniSleeperThreadUsage) {
		...
		if (!this.p.a() && !eulaAgreed) { // Spigot
			...
			if (!this.getOnlineMode()) {
				...
				if (org.spigotmc.SpigotConfig.bungee) {
					...
					if (!AerosConfig.stopNotifyBungee) {
						DedicatedServer.LOGGER
								.warn("---------------------------- Aeros Checker ----------------------------");
						DedicatedServer.LOGGER.warn(
			...
			if (!NameReferencingFileConverter.a(this.propertyManager)) {
				...
				this.windSpigot = new Aeros(); // Aeros
