...
public class EntityTNTPrimed extends Entity {
	...
	public boolean W() {
		...
		/*
		 * // Preserve velocity while calling the super method double oldMotX =
		 * this.motX; double oldMotY = this.motY; double oldMotZ = this.motZ;
		 * 
		 * super.W();
		 * 
		 * this.motX = oldMotX; this.motY = oldMotY; this.motZ = oldMotZ;
		 * 
		 * if (this.inWater) { // Send position and velocity updates to nearby players
		 * on every tick while the TNT is in water. // This does pretty well at keeping
		 * their clients in sync with the server. EntityTrackerEntry ete =
		 * ((WorldServer)
		 * this.getWorld()).getTracker().trackedEntities.get(this.getId()); if (ete !=
		 * null) { PacketPlayOutEntityVelocity velocityPacket = new
		 * PacketPlayOutEntityVelocity(this); PacketPlayOutEntityTeleport positionPacket
		 * = new PacketPlayOutEntityTeleport(this);
		 * 
		 * for (EntityPlayer viewer : ete.trackedPlayers) { if ((viewer.locX -
		 * this.locX) * (viewer.locY - this.locY) * (viewer.locZ - this.locZ) < 16 * 16)
