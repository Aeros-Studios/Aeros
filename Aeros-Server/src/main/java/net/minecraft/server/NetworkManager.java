...
import dev.aeros.exception.ExploitException;
...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
	...
	protected void a(ChannelHandlerContext ctx, Packet packet) throws Exception {
		if (this.channel.isOpen()) {
			if (this.m instanceof PlayerConnection) {
				try {
					for (dev.aeros.protocol.PacketListener packetListener : Aeros.getInstance()
							.getPacketListeners()) {
	...
	public void a(PacketListener packetlistener) {
		...
		// NetworkManager.g.debug("Set listener of {} to {}", new Object[] { this,
		// packetlistener});
		...
		if (this.isConnected()) {
			...
			if (!shouldCheckPacket) {
				...
				if (AerosConfig.asyncKnockback
						&& (packet instanceof PacketPlayOutEntityVelocity || packet instanceof PacketPlayOutPosition
								|| packet instanceof PacketPlayInFlying.PacketPlayInPosition
								|| packet instanceof PacketPlayInFlying)) {
					// Send it with high priority
					Aeros.getInstance().getKnockbackThread().addPacket(packet, this, null);
					return;
				}
			}
			// Aeros end
