...
public abstract class EntityHuman extends EntityLiving {
	...
	protected void b(ItemStack itemstack, int i) {
		...
		if (itemstack.m() == EnumAnimation.EAT) {
			for (int j = 0; j < i; ++j) {
				...
				Vec3D vec3d = new Vec3D((this.random.nextFloat() - 0.5D) * 0.1D, random.nextDouble() * 0.1D + 0.1D,
						0.0D);
	...
	protected boolean d(DamageSource damagesource, float f) { // void -> boolean
		 ...
		 * f = (1.0F + f) * this.world.paperSpigotConfig.playerBlockingDamageMultiplier;
		 * // PaperSpigot -
		 * // Configurable damage
		 * // multiplier for
		 * // blocking;
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					if (flag2) {
						if (i > 0) {
							...
							// Aeros start - more configurable knockback
							if (this.isExtraKnockback()) {
								entity.g(
										(-MathHelper.sin((float) (this.yaw * Math.PI / 180.0D)) * i
												* profile.getWTapExtraHorizontal()),
										profile.getWTapExtraVertical(),
										(MathHelper.cos((float) (this.yaw * Math.PI / 180.0D))
