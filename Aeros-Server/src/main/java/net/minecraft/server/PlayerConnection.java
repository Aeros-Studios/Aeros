...
import dev.aeros.events.PlayerIllegalBehaviourEvent;
...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
	...
	public void c() {
		...
		if (AerosConfig.modernKeepalive) {
		...
		if (this.b(packetplayinflying)) {
			...
			if (!this.player.viewingCredits) {
				...
				if ((packetplayinflying.hasPos) && ((delta > 0.0D) && (this.checkMovement && !this.player.dead))) {
					for (dev.aeros.protocol.MovementListener movementListener : Aeros.getInstance()
				...
				if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
					...
					if (AerosConfig.firePlayerMoveEvent
							&& PlayerMoveEvent.getHandlerList().getRegisteredListeners().length != 0) { // Aeros -
																										// only handle
																										// player move
																										// event when
																										// there are
						...
						if (event.isCancelled()) {
							Aeros.getInstance().getLagCompensator().registerMovement(player, to); // Nacho
					...
					Aeros.getInstance().getLagCompensator().registerMovement(player, to); // Nacho - register movement
				}
				...
				if (d3 > 36.0D) {
					...
					if (packetplayinblockdig.c() == PacketPlayInBlockDig.EnumPlayerDigType.START_DESTROY_BLOCK) {
						if (!this.minecraftServer.a(worldserver, blockposition, this.player)
							...
							CraftEventFactory.callPlayerInteractEvent(this.player, Action.LEFT_CLICK_BLOCK,
									blockposition,
	...
	public void a(PacketPlayInBlockPlace packetplayinblockplace) {
		...
		if (packetplayinblockplace.getFace() == 255) {
			...
			if (!throttled) {
				...
				// SportPaper end
			} else if (MinecraftServer.currentTick - lastDropTick > 1 && AerosConfig.fixEatWhileRunning) {
				this.player.playerInteractManager.useItem(this.player, this.player.world, itemstack);
			}
		...
		if (itemstack == null || itemstack.l() == 0) {
			...
			// KigPaper-0172
			if (AerosConfig.stopDecodingItemStackOnPlace
	...
	public void sendPacket(final Packet packet) {
		...
		try {
			for (dev.aeros.protocol.PacketListener packetListener : Aeros.getInstance().getPacketListeners()) {
				try {
	...
	public void a(PacketPlayInUseEntity packetplayinuseentity) {
		...
		if (entity != null) {
			...
			if (!flag) {
				...
				d0 = (AerosConfig.improvedHitDetection) ? 12.75D : 9.0D;
			} else {
				// Aeros start - configurable reach cap
				if (AerosConfig.creativeBypass
						&& this.player.playerInteractManager.getGameMode() == EnumGamemode.CREATIVE) {
					d0 = 36.0;
				} else {
					d0 = AerosConfig.maxReachSqrd;
				}
			}
			...
			case PERFORM_RESPAWN:
				if (this.player.viewingCredits) {
					...
					if (this.minecraftServer.T() && this.player.getName().equals(this.minecraftServer.S())) {
						...
						GameProfileBanEntry gameprofilebanentry = new GameProfileBanEntry(this.player.getProfile(),
								null,
	...
	public void a(PacketPlayInSetCreativeSlot packetplayinsetcreativeslot) {
		...
		// {
		// System.out.println(this.player.getName() + " has fired a sprocky debug Code
		// (might fire for 1.9+ too) [Creative slot] 0xLF36 ");
		// }
		...
		} else if (AerosConfig.kickOnIllegalBehavior) {
	...
	// Aeros start - queue-able packets
