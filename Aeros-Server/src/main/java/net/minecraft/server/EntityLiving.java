...
public abstract class EntityLiving extends Entity {
	...
	public EntityLiving(World world) {
		...
		this.k = true;
		// Aeros start - use faster randoms
	...
	public boolean damageEntity(DamageSource damagesource, float f) {
		if (this.isInvulnerable(damagesource)) {
			...
			if (this.getHealth() <= 0.0F) {
				...
				/*
				 * if (false && (damagesource == DamageSource.ANVIL || damagesource ==
				 * DamageSource.FALLING_BLOCK)
				 * && this.getEquipment(4) != null) {
				 * this.getEquipment(4).damage((int) (f * 4.0F + this.random.nextFloat() * f *
				 * 2.0F), this);
				 * f *= 0.75F;
				 * }
				 */
				...
				if (flag && !(knockbackCancelled = world.paperSpigotConfig.disableExplosionKnockback
					...
					if (entity != null) {
						...
						for (distanceZ = entity.locZ - this.locZ; distanceX * distanceX + distanceZ
								* distanceZ < 1.0E-4D; distanceZ = (random.nextDouble() - random.nextDouble())
										* 0.01D) {
	...
	public void a(double x, double z, DamageSource source) {
		if (this.random.nextDouble() >= this.getAttributeInstance(GenericAttributes.c).getValue()) {
			...
			if (source instanceof EntityDamageSourceIndirect) {
				...
				} else if (((EntityDamageSourceIndirect) source)
						.getProximateDamageSource() instanceof EntityEnderPearl) {
			...
			// Aeros start - correct knockback friction (change to division instead of
			// multiplication)
			...
			// Aeros start - knockback addition config
	...
	protected float applyMagicModifier(DamageSource damagesource, float f) {
		if (damagesource.isStarvation()) {
			...
			/*
			 * if (false && this.hasEffect(MobEffectList.RESISTANCE) && damagesource !=
	...
	public void m() {
		...
		// Aeros start - smoother mob AI disable
	...
	public boolean hasLineOfSightAccurate(Entity entity) {
		...
		if (entity instanceof EntityPlayer && AerosConfig.improvedHitDetection) {
