...
public abstract class World implements IBlockAccess {
	...
	public final List<Entity> entityList = new ObjectArrayList<Entity>() { // Aeros - ArrayList -> ObjectArrayList
		@Override
	...
	public final List<EntityHuman> players = new ObjectArrayList<>(); // Aeros - ArrayList -> ObjectArrayList
	public final List<Entity> k = new ObjectArrayList<>(); // Aeros - ArrayList -> ObjectArrayList
	...
	protected boolean lastTickOverload = false; // Aeros
	...
	protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider,
		...
		timings = new co.aikar.timings.WorldTimingsHandler(this); // Spigot - code below can generate new world and
																	// access timings
		// Aeros - re-implement Spigot's entity max tick time, but only for certain
		// entities
		this.entityLimiter = new dev.aeros.entity.EntityTickLimiter(AerosConfig.entityMaxTickTime); // new
																									// org.spigotmc.TickLimiter(spigotConfig.entityMaxTickTime);
		this.tileLimiter = new org.spigotmc.TickLimiter(AerosConfig.tileMaxTickTime);
	}
	...
	public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
		...
		if (!flag && !this.isChunkLoaded(i, j, true)) {
			...
			// Aeros start - configurable entity hit delay
			if (entity instanceof EntityLiving) {
				((EntityLiving) entity).maxNoDamageTicks = AerosConfig.hitDelay;
			}
			// Aeros end

			// Aeros start - configurable potion speeds
			if (entity instanceof EntityPotion) {
				// Subtract the speed from the y value of the velocity, the potion will drop
				// faster and will therefore splash faster
				entity.motY -= AerosConfig.potionSpeed;

				// Mark velocity as changed only if potion speed is not default
				if (!(AerosConfig.potionSpeed == 0)) {
	...
	public void removeEntity(Entity entity) {
		...
		// if (!guardEntityList) { // Spigot - It will get removed after the tick if we
		// are ticking // Paper - moved down
		int i = entity.ae;
		int j = entity.ag;
	...
	public void tickEntities() {
		boolean tickOverload = false; // Aeros
		...
		entityLimiter.initTick(); // Aeros - re-implement Spigot's entity max tick time, but only for certain
									// entities
		for (tickPosition = 0; tickPosition < entityList.size(); tickPosition++) {
			...
			// Aeros - synchronize
			synchronized (entity) {
				// Aeros start - re-implement Spigot's entity max tick time, but only for
				// certain entities
		...
		lastTickOverload = tickOverload; // Aeros
	}
	...
	public float a(Vec3D vec3d, AxisAlignedBB axisalignedbb) {
		...
		// Aeros start - switch to mathhelper instead of Java math
