...
import dev.aeros.random.FastRandom;
...
import dev.aeros.tickloop.ReentrantIAsyncHandler;
import dev.aeros.tickloop.TasksPerTick;
import dev.aeros.world.WorldTickManager;
// Aeros end

public abstract class MinecraftServer extends ReentrantIAsyncHandler<TasksPerTick>
		implements ICommandListener, IAsyncTaskHandler, IMojangStatistics {
	...
	public java.util.Queue<Runnable> priorityProcessQueue = new java.util.concurrent.ConcurrentLinkedQueue<Runnable>(); // Aeros
	public int autosavePeriod;
	// CraftBukkit end

	// Aeros - instance
	protected Aeros windSpigot;

	// Aeros - MSPT for tps command
	private double lastMspt;

	// Aeros start - backport modern tick loop
	...
	public MinecraftServer(OptionSet options, Proxy proxy, File file1, Thread thread) {
		super("Server"); // Aeros - backport modern tick loop
	...
	private static final java.math.BigDecimal TPS_BASE = new java.math.BigDecimal(1E9)
			.multiply(new java.math.BigDecimal(SAMPLE_INTERVAL));
	// Paper End

	private AffinityLock lock = null;

	// Aeros - thread affinity
	...
	public void run() {
		...
		try {
			...
			if (this.init()) {
				...
				// Aeros start - implement thread affinity
				if (AerosConfig.threadAffinity) {
					LOGGER.info(" ");
					...
					Aeros.debug(AffinityLock.dumpLocks());

				}
				// Aeros end

				// Aeros - parallel worlds
				...
				long start = System.nanoTime(), curTime, tickSection = start; // Paper - Further improve server tick
																				// loop
				...
				while (this.isRunning) {
					...
					if (i > 5000L && this.nextTickTime - this.lastOverloadWarning >= 30000L && ticks > 500) { // CraftBukkit
						...
						if (this.server.getWarnOnOverload()) // CraftBukkit
							MinecraftServer.LOGGER.warn(
									"Can't keep up! Is the server overloaded? Running {}ms or {} ticks behind", i, j);
					...
					if (++MinecraftServer.currentTick % MinecraftServer.SAMPLE_INTERVAL == 0) {
						...
						java.math.BigDecimal currentTps = TPS_BASE.divide(new java.math.BigDecimal(diff), 30,
			...
			// Aeros start - thread affinity
			if (lock != null) {
				lock.release();
				MinecraftServer.LOGGER.info("Released CPU " + lock.cpuId() + " from server usage.");
			}
			// Aeros end
			// Aeros start - stop statistics connection
			Thread statisticsThread = null;
			if (disableStatistics) {
				StatisticsClient client = this.getAeros().getClient();
			...
			// Aeros - wait for statistics to finish stopping
			try {
				if (this.getAeros().getClient().isConnected) {
	...
	private boolean haveTime() {
		...
		return this.forceTicks || this.runningTask()
				|| getMillis() < (this.mayHaveDelayedTasks ? this.delayedTasksMaxNextTickTime : this.nextTickTime);
	}
	...
	protected TasksPerTick packUpRunnable(Runnable runnable) {
		// Paper start - anything that does try to post to main during watchdog crash,
		// run on watchdog
	...
	// Aeros - backport modern tick loop
	protected void A(BooleanSupplier shouldKeepTicking) throws ExceptionWorldConflict { // CraftBukkit - added throws
		...
		this.server.getPluginManager()
				.callEvent(new com.destroystokyo.paper.event.server.ServerTickStartEvent(this.ticks + 1)); // Paper
		// Aeros end
		...
		this.server.getPluginManager().callEvent(
				new com.destroystokyo.paper.event.server.ServerTickEndEvent(this.ticks, this.lastMspt, remaining));
		// Paper end
		// Aeros end
		...
		// if (false && getSnooperEnabled() && !this.n.d() && this.ticks > 100) { //
		// Spigot
		// this.n.a();
		// }
		//
	...
	public void B() {
		...
		// Aeros - move to WorldTickManager
		...
		// Aeros start - priority process queue
	...
	public String getServerModName() {
		return AerosConfig.serverBrandName; // [Nacho-0035] // NachoSpigot - NachoSpigot > // TacoSpigot -
											// TacoSpigot //
	...
	// Aeros start - backport modern tick loo
	...
	// Aeros - MSPT (milliseconds per tick)
