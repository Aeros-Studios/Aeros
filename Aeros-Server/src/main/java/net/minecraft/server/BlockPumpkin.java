...
import dev.aeros.cache.Constants;
...
public class BlockPumpkin extends BlockDirectional {
	...
	private static final Predicate<IBlockData> Q = new Predicate() {
		public boolean a(IBlockData iblockdata) {
			return iblockdata != null
					&& (iblockdata.getBlock() == Blocks.PUMPKIN || iblockdata.getBlock() == Blocks.LIT_PUMPKIN);
		}
	...
	protected BlockPumpkin() {
		...
		this.j(this.blockStateList.getBlockData().set(BlockDirectional.FACING, EnumDirection.NORTH));
		this.a(true);
	...
	public boolean e(World world, BlockPosition blockposition) {
		return this.getDetectorSnowGolemPart().a(world, blockposition) != null
				|| this.getDetectorIronGolemPart().a(world, blockposition) != null;
	}
	...
	private void f(World world, BlockPosition blockposition) {
		...
		if ((shapedetector_shapedetectorcollection = this.getDetectorSnowGolem().a(world, blockposition)) != null) {
			BlockStateListPopulator blockList = new BlockStateListPopulator(world.getWorld()); // CraftBukkit - Use
			...
			if (world.addEntity(entitysnowman, SpawnReason.BUILD_SNOWMAN)) {
				...
				for (j = 0; j < 120; ++j) {
					world.addParticle(EnumParticle.SNOW_SHOVEL, blockposition1.getX() + world.random.nextDouble(),
							...
							blockposition1.getZ() + world.random.nextDouble(), 0.0D, 0.0D, 0.0D, Constants.EMPTY_ARRAY);
				}
		...
		} else if ((shapedetector_shapedetectorcollection = this.getDetectorIronGolem().a(world,
				blockposition)) != null) {
			BlockStateListPopulator blockList = new BlockStateListPopulator(world.getWorld()); // CraftBukkit - Use
																								// BlockStateListPopulator
			for (i = 0; i < this.getDetectorIronGolem().c(); ++i) {
				for (int k = 0; k < this.getDetectorIronGolem().b(); ++k) {
					// CraftBukkit start
					// world.setTypeAndData(shapedetectorcollection.a(i, k, 0).d(),
			...
			if (world.addEntity(entityirongolem, SpawnReason.BUILD_IRONGOLEM)) {
				...
				for (j = 0; j < 120; ++j) {
					world.addParticle(EnumParticle.SNOWBALL, blockposition2.getX() + world.random.nextDouble(),
							blockposition2.getY() + world.random.nextDouble() * 3.9D,
							blockposition2.getZ() + world.random.nextDouble(), 0.0D, 0.0D, 0.0D, Constants.EMPTY_ARRAY);
				}
	...
	public IBlockData fromLegacyData(int i) {
		return this.getBlockData().set(BlockDirectional.FACING, EnumDirection.fromType2(i));
	}

	@Override
	public int toLegacyData(IBlockData iblockdata) {
		return iblockdata.get(BlockDirectional.FACING).b();
	}
	...
	protected ShapeDetector getDetectorSnowGolemPart() {
		if (this.snowGolemPart == null) {
			this.snowGolemPart = ShapeDetectorBuilder.a().a(new String[] { " ", "#", "#" })
	...
	protected ShapeDetector getDetectorSnowGolem() {
		if (this.snowGolem == null) {
			this.snowGolem = ShapeDetectorBuilder.a().a(new String[] { "^", "#", "#" })
	...
	protected ShapeDetector getDetectorIronGolemPart() {
		if (this.ironGolemPart == null) {
			this.ironGolemPart = ShapeDetectorBuilder.a().a(new String[] { "~ ~", "###", "~#~" })
	...
	protected ShapeDetector getDetectorIronGolem() {
		if (this.ironGolem == null) {
			this.ironGolem = ShapeDetectorBuilder.a().a(new String[] { "~^~", "###", "~#~" })
					.a('^', ShapeDetectorBlock.a(BlockPumpkin.Q))
					.a('#', ShapeDetectorBlock.a(BlockStatePredicate.a(Blocks.IRON_BLOCK)))
					.a('~', ShapeDetectorBlock.a(BlockStatePredicate.a(Blocks.AIR))).b();
		}
