...
public class NachoConfig {
	...
	public static void init(File configFile) {
		/*
		 * if (!configFile.exists()) { TODO: uncomment this after config migration is
		 * finished
		 * hasMigrated = true;
		 * return;
		 * }
		...
		try {
			Aeros.LOGGER.info("Loading NachoSpigot config from " + configFile.getName());
	...
	// Migrates all read data into the Aeros config
	// TODO: save this data to file instead of just putting it in memory
	public static void moveToAerosConfig(Class<?> clazz, Class<?> windSpigotConfigClazz) {
		for (Field oldField : clazz.getDeclaredFields()) {

			if (Modifier.isPublic(oldField.getModifiers()) && Modifier.isStatic(oldField.getModifiers())
					&& !oldField.getName().equals("CONFIG_FILE") && !oldField.getName().equals("hasMigrated")) {
				...
				if (!found) {
					Aeros.LOGGER.warn("Unable to migrate option " + oldField.getName() + "!");
				}
			}
		}
		hasMigrated = true;
