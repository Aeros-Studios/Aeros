package dev.aeros.hitdetection;
...
public class LagCompensator {
    ...
    /*
     * Lag Compensator based on
     * https://github.com/Islandscout/ServerSideHitDetection
    ...
    private void processPosition(Location loc, Player p) {
        if (!AerosConfig.improvedHitDetection)
            return;
        ...
        if (timesSize > 0
                && currTime - locationTimes.get(p.getUniqueId()).get(timesSize - 1).getValue() < timeResolution)
            return;
