package dev.aeros.async.entitytracker;
...
public class MultithreadedEntityTracker extends EntityTracker {
	...
	private static final ExecutorService trackingThreadExecutor = Executors
			.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("Aeros Entity Tracker Thread").build());
	...
	public void updatePlayers() {
		...
		for (int i = 1; i <= AerosConfig.trackingThreads; i++) {
			final int finalOffset = offset++;

			AsyncUtil.run(() -> {
				for (int index = finalOffset; index < c.size(); index += AerosConfig.trackingThreads) {
