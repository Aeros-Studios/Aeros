...
public class PingCommand extends Command {
	...
	public boolean execute(CommandSender sender, String currentAlias, String[] args) {
		...
		if ((args.length == 0) && sender instanceof Player) {
			String finalString = ChatColor.translateAlternateColorCodes('&', AerosConfig.pingSelfCmdString)
			...
			if (pingPlayer != null && Bukkit.getOnlinePlayers().contains(pingPlayer)) {
				String finalString = ChatColor.translateAlternateColorCodes('&', AerosConfig.pingOtherCmdString)
