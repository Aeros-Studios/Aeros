...
import dev.aeros.async.pathsearch.AsyncNavigation;
import dev.aeros.entity.EntityTickLimiter;

@SuppressWarnings("unused")
public class AerosConfig {

	private static final Logger LOGGER = LogManager.getLogger(AerosConfig.class);
	public static File CONFIG_FILE;
	protected static final YamlCommenter c = new YamlCommenter();
	private static final String HEADER = "This is the main configuration file for Aeros.\n"
			+ "As you can see, there's tons to configure. Some options may impact gameplay, so use\n"
			+ "with caution, and make sure you know what each option does before configuring.\n" + "\n"
			+ "If you need help with the configuration or have any questions related to Aeros,\n"
	...
	public static void init(File configFile) {
		...
		try {
			Aeros.LOGGER.info("Loading Aeros config from " + configFile.getName());
	...
	// Not private as the config is read by calling all private methods with 0
	// params
	...
	static void loadComments() {
		...
		c.addComment("settings.pearl-passthrough",
				"Configuration for ender pearls passing through certain blocks. (Credits to FlamePaper)");
		c.addComment("settings.command", "Configuration for Aeros's commands");
		...
		c.addComment("settings.async.entity-tracking.full-async",
				"Enables a faster, fully async tracker. May have issues with plugins");
		c.addComment("settings.async.entity-tracking.threads",
				"The amount of threads used for async entity tracking per world, increase or decrease this based on your server load.");
		...
		c.addComment("settings.modern-keep-alive",
		...
		c.addComment("settings.async.path-searches.entities",
		...
		c.addComment("settings.max-tick-time.skippable-entities",
		...
		c.addComment("settings.max-reach.value",
		...
		c.addComment("settings.spawners.max-nearby-entities",
				"Maximum number of nearby entities to avoid new spawns (Any value lower than 1 will disable this check)");
		c.addComment("settings.spawners.required-player-range",
				"Area in which the player is required to spawn entities");
		...
		c.addComment("settings.anti-crash.enabled",
		...
		c.addComment("settings.faster-cannon-tracker",
				"Enables a faster cannon entity tracker. Please note that this can cause some incompatibilities with name tags for cannon entities.");
		...
		if (asyncPathSearches) {
			List<String> asyncSearchEntities = getList("settings.async.path-searches.entities",
					Lists.newArrayList("BAT", "BLAZE", "CHICKEN", "COW", "CREEPER", "ENDERMAN", "HORSE", "IRON_GOLEM",
							"MAGMA_CUBE", "MUSHROOM_COW", "PIG", "PIG_ZOMBIE", "RABBIT", "SHEEP", "SKELETON",
							"SILVERFISH",
	...
	private static void maxTickTimes() {
		...
		tileMaxTickTime = 1000; // We do not re-implement the tile entity tick cap, so we disable it by setting
								// it to 1000
	}
	...
	private static void serverBrandName() {
		serverBrandName = getString("settings.brand-name", "Aeros");
	}
